...
No two vehicles share a Design Identity keyword within the same cluster — checked across the whole Mainstream family minivan cluster (VOXY/NOAH/SERENA/STEPWGN/ESQUIRE), the Luxury minivan cluster (Alphard/Vellfire), the Compact minivan cluster (FREED/シエンタ), the SUV-influenced minivan cluster (デリカD:5, sole member), and the Compact tall wagon cluster (ソリオ, sole member) separately, not only within one maker — and every Avoid list either mirrors a cluster-mate's own keywords (catching collisions structurally, not just at review time) or is sourced from an official contrast. (Vellfire's "Aggressive" is not a collision with VOXY's own "Aggressive" — they sit in different clusters, per the cluster-scoped check this document itself defines.)
