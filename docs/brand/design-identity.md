...
| Compact minivan | FREED, シエンタ | FREED (added 2026-07) is genuinely smaller-class, not a Mainstream-cluster member despite also being a Honda alongside STEPWGN -- see "What Reference Length is" below, which predicted almost exactly this addition ("a future smaller-class... around 4250-4300mm") before FREED existed. シエンタ (added 2026-07-31) is the predicted addition itself, at 4,260mm -- essentially the same size class as FREED and a real-world cross-shopped rival, not a coincidence. |
| SUV-influenced minivan | デリカD:5 | デリカD:5 (added 2026-07-31) fuses minivan comfort with genuine off-road capability (185mm ground clearance, 21.0°/23.0° approach/departure angles) -- no other CarVista vehicle competes in this space. Sole member so far. |
| Compact tall wagon | ソリオ | ソリオ (added 2026-07-31) is CarVista's smallest vehicle yet (3,810mm), a tall wagon rather than a minivan in body type -- not a Mainstream/Compact-minivan member despite superficially similar proportions. Sole member so far. |
| Kei super-height wagon | N-BOX, SPACIA | N-BOX and SPACIA (added 2026-08-03) are kei-jidousha (660cc, legally capped at 3,395mm/1,480mm), a fundamentally different regulatory class from every other CarVista vehicle including ソリオ -- not a size-only distinction. The two are Japan's best-selling kei tall wagons and genuine direct rivals, so they get their own cluster rather than joining ソリオ's Compact tall wagon row despite superficial body-shape similarity. |
...
| デリカD:5 | Mitsubishi | The rugged all-rounder. | Rugged, Functional, Adventurous | Elegant, Luxury, Refined | CarVista's first Mitsubishi vehicle and sole member of the new SUV-influenced minivan cluster. Mitsubishi's own stated development theme fuses "ミニバンの優しさ" (minivan comfort) with "SUVの力強さ" (SUV strength/capability), backed by real off-road figures (185mm ground clearance, 21.0°/23.0° approach/departure angles) — not a styling gesture. |
| ソリオ | Suzuki | The practical everyday choice. | Practical, Efficient, Right-sized | Aggressive, Luxury, Bold | CarVista's first Suzuki vehicle and sole member of the new Compact tall wagon cluster — its own segment, smaller even than FREED. Suzuki's own current-generation direction: extend length for cargo room while holding a class-leading 4.8m turning radius and a body width deliberately kept narrow for parking/handling ease. |
| シエンタ | Toyota | The everyday people-mover. | Rounded, Practical, Unassuming | Aggressive, Luxury, Angular | Joins FREED in the Compact minivan cluster (4,260mm, a real-world cross-shopped rival) — checked keyword-by-keyword against FREED's own Approachable/Compact/Cheerful and found to differ in vocabulary family, not just wording: FREED's is an attitude/warmth register, Sienta's is a form/utility register sourced from Toyota's own "シカクマル" (square+circle) motif and "日常に溶け込む道具的な機能美" (everyday tool-like functional beauty). |
| N-BOX | Honda | The everyone's kei car. | Friendly, Approachable, Universal | Aggressive, Industrial, Exclusive | First member of the new Kei super-height wagon cluster (with Spacia). Sourced from Honda's own 3rd-gen grand concept ("ハッピー・リズム・ボックス", theme "みんなのN") and design interview: "愛嬌がありながら、でも凛々しい...水平基調のデザインが乗る人すべてをやさしく包む一体感" — a warm, universally-approachable register. |
| SPACIA | Suzuki | The playful utility box. | Playful, Sturdy, Adventurous | Fragile, Formal, Universal | Joins N-BOX in the new Kei super-height wagon cluster — the two are Japan's best-selling kei tall wagons and real direct rivals, so the vocabulary families were deliberately kept apart: N-BOX's is warmth/roundness, Spacia's is robust-container/playful-freedom, sourced from Suzuki's own "わくわく満載！自由に使える" concept and "頑丈で大容量のコンテナをモチーフ...工業製品のような角を面取りしたフォルム" design language. |

No two vehicles share a Design Identity keyword within the same cluster — checked across the whole Mainstream family minivan cluster (VOXY/NOAH/SERENA/STEPWGN/ESQUIRE), the Luxury minivan cluster (Alphard/Vellfire), the Compact minivan cluster (FREED/シエンタ), the SUV-influenced minivan cluster (デリカD:5, sole member), the Compact tall wagon cluster (ソリオ, sole member), and the Kei super-height wagon cluster (N-BOX/SPACIA) separately, not only within one maker — and every Avoid list either mirrors a cluster-mate's own keywords (catching collisions structurally, not just at review time) or is sourced from an official contrast. (Vellfire's "Aggressive" is not a collision with VOXY's own "Aggressive" — they sit in different clusters, per the cluster-scoped check this document itself defines.)
...
| デリカD:5 | G | ダイヤモンドホワイトパール | "G" is the current lineup's base grade (vs. P / G-Power Package / LUXFZ-LUDFZ) -- standard, not loaded, matching Functional. Diamond White Pearl is sourced as the current model's most popular color and matches both generation photos. **Exception**: Hero uses the original black デリカTOP.png as-is, by explicit decision -- its atmospheric studio lighting reads closer to VOXY/SERENA's own Hero photos than any available white alternative, and black-to-white recoloring was tried and rejected (see src/cars/delica-d5.js's own comment). Hero and the generation photos are knowingly different colors; not a bug. |
| ソリオ | HYBRID MX | ピュアホワイトパール | "HYBRID MX" is the mid/standard grade in the current 3-grade lineup (MG/MX/MZ) -- not base, not loaded. Pure White Pearl is sourced as this vehicle's most popular color. gen1's originally-supplied beige/champagne photo was recolored (hue/saturation remapped onto the original lightness/shading) to match. |
| シエンタ | G | アーバンカーキ | "G" is the current lineup's best-selling standard grade (vs. entry X / top Z). アーバンカーキ is sourced as a real Toyota color name and matches the Hero and gen3 photos. gen1 (originally yellow) and gen2 (originally gold) were recolored the same way as ソリオ's gen1. gen2's black roof panel was left untouched (see the data-file comment in src/cars/sienta.js) -- a minor two-tone remainder, not a full match. |
| N-BOX | G | プレミアムサンライトホワイト・パール | "G" is the base/standard grade in the current G/L/EX lineup (Custom is a separate trim line, not depicted). The color is a real, current-generation (JF5/JF6) Honda name, sourced as a "beautiful white shining like sunlight" register, matching every supplied photo and reinforcing Friendly/Approachable. |
| SPACIA | HYBRID X | ピュアホワイトパール | "HYBRID X" is the fuller-equipped mainstream grade (vs. entry HYBRID G), with Suzuki Safety Support and both power slide doors standard. ピュアホワイトパール is Suzuki's current most-popular color (30.3% share, 2026 sales data) and matches every supplied photo. |
...
| デリカD:5 | ~4790mm | SUV-influenced minivan — its own cluster; numerically close to STEPWGN but a different vehicle type entirely (real off-road capability, not just length), so no direct scale comparison to the Mainstream cluster is meaningful. |
| シエンタ | ~4260mm | Compact minivan — near-identical size class to FREED (4,310mm), reinforcing the cluster fit above. |
| ソリオ | ~3810mm | Compact tall wagon — CarVista's smallest vehicle, should read visibly smaller in frame than even the Compact minivan cluster. |
| N-BOX | ~3395mm | Kei super-height wagon — the kei-jidousha regulatory length ceiling; tied with SPACIA as CarVista's smallest vehicles yet, visibly smaller in frame than even ソリオ. |
| SPACIA | ~3395mm | Kei super-height wagon — same kei-jidousha regulatory length ceiling as N-BOX; the two should read as the same size class as each other. |
...
## Continuously-produced vehicles with no full model change (デリカD:5)

デリカD:5 (added 2026-07-31) is a different case from ESQUIRE above: not discontinued, and not literally single-generation either — it has been continuously produced since January 2007 under one model change, with a genuinely major 2019 exterior redesign the press itself calls a "ビッグマイナーチェンジ," and further running updates since (most recently January 2026). Two real, distinct-looking photos exist (pre-2019 and post-2019 faces), and Timeline/Heritage/Compare are all meaningful for it — unlike ESQUIRE, its HTML is **not** stripped down.

- **`generations` has two entries, but they are NOT labeled `初代`/`2代目`.** Doing so would misrepresent a single continuously-produced model as two full generations — exactly the kind of error a car-enthusiast reader would catch immediately. Instead they use `era: '前期'` / `era: '後期'` (early-type/late-type), real terminology this vehicle's own press and owner community already use for exactly this pre/post-2019 split.
- **A FAQ entry states this directly** (`faq[0]` in `src/cars/delica-d5.js`) for any reader who arrives expecting a normal generation history and needs the correction up front, not buried in a code comment only a future editor would see.
- **The 2026-01 running update is folded into gen2's own `facelift` field**, not treated as a third generation-like entry — `facelift` already models "this generation changed mid-cycle," which is exactly what happened.
- If a future vehicle turns out to have this same shape (one continuous model, a press-recognized major face change, no formal generation renumbering), reuse this pattern rather than either forcing it into `初代`/`2代目` or collapsing it to ESQUIRE's single-entry treatment.
