...
| デリカD:5 | G | ダイヤモンドホワイトパール | "G" is the current lineup's base grade (vs. P / G-Power Package / LUXFZ-LUDFZ) -- standard, not loaded, matching Functional. Diamond White Pearl is sourced as the current model's most popular color and matches both generation photos. Hero reuses the gen2 (後期) source photo, recropped, instead of the originally-supplied black example -- avoids a color mismatch without any recolor risk. |
